...
import { TorrentSearchIndex, parseQuery } from "../src/lib/filter.js";
import { IncrementalTorrentSorter } from "../src/lib/sort.js";

describe("filter", () => {
  const torrent = {
    hash: "ABCDEF012345",
    name: "Ubuntu Server",
    basePath: "/downloads/linux",
  ...
  it("search index matches, re-indexes on update, and forgets on remove", () => {
    ...
    assert.equal(
      index.matches(
        torrent,
        parseQuery(
          "ubuntu abc path:linux tracker:example label:iso status:active ratio>1 size<4gb",
        ),
      ),
      true,
    );
  ...
  it("incremental sorter orders by name, tracking renames", () => {
    ...
    assert.deepEqual(
      sorter.sort([beta, alpha], [{ column: "name", direction: "asc" }]).map((item) => item.hash),
      ["A", "B"],
    );
    const moved = { ...beta, name: "0 Beta" };
    assert.deepEqual(
      sorter.sort([moved, alpha], [{ column: "name", direction: "asc" }]).map((item) => item.hash),
      ["B", "A"],
    );
    assert.deepEqual(
      sorter

...
function TrackersTab(props: { hash: string; detail: TorrentDetail }) {
  ...
  const add = () => { const url = window.prompt("Tracker URL:"); if (!url) return; try { const parsed = new URL(url); if (!/^https?:$/.test(parsed.protocol)) throw new Error(); } catch { showToast("Enter a valid http(s) tracker URL."); return; } void detailAction("tracker_add", props.hash, { trackerUrl: url }).then(() => { showToast("Tracker added."); void fetchDetail(props.hash); }).catch((error: Error) => showToast(error.message)); };
  ...
  const remove = (tracker: Tracker) => { if (!window.confirm(`Disable tracker ${tracker.url}?`)) return; void detailAction("tracker_enable", props.hash, { trackerIndex: tracker.index, enabled: false }).then(() => { showToast("Tracker disabled."); void fetchDetail(props.hash); }).catch((error: Error) => showToast(error.message)); };
  const trackerRows = () => props.detail.trackers;
  return <div class="trackers-tab"><div class="tracker-list"><For each={trackerRows()}>{(tracker) => <div class="tracker-row"><i classList={{ off: !tracker.isEnabled }} /><span class="tracker-url" title={tracker.url}>{tracker.url}</span><span class="tracker-status" classList={{ off: !tracker.isEnabled }}>{tracker.isEnabled ? "Working" : "Disabled"}</span><span class="tnum">{tracker.seeds} / {tracker.leechers}</span><span class="tnum">{next(tracker)}</span><button class="tracker-remove" type="button" onClick={() => remove(tracker)}>Remove</button></div>}</For><div class="tracker-row pseudo"><i /><span class="tracker-url">[DHT]</span><span class="tracker-status">Enabled</span><span>—</span><span>—</span></div><div class="tracker-row pseudo"><i /><span class="tracker-url">[Peer exchange]</span><span class="tracker-status">Enabled</span><span>—</span><span>—</span></div></div><footer class="tracker-actions"><button type="button" onClick={add}>Add tracker</button><button type="button" onClick={announce}>Force reannounce</button></footer></div>;
}

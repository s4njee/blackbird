...
export type ParsedQuery = { terms: string[]; labels: string[]; trackers: string[]; paths: string[]; statuses: string[]; comparisons: Comparison[] };

type SearchEntry = { name: string; hash: string; path: string; tracker: string; message: string; label: string };
...
function parseSize(value: string) {
  ...
  const units: Record<string, number> = { b: 1, kb: 1024, kib: 1024, mb: 1024 ** 2, mib: 1024 ** 2, gb: 1024 ** 3, gib: 1024 ** 3, tb: 1024 ** 4, tib: 1024 ** 4 };
  return Number(match[1]) * (units[match[2] || "b"] ?? 1);
}

/** Parses AND-combined free-text terms and the documented filter prefixes. */
export function parseQuery(input: string): ParsedQuery {
  const result: ParsedQuery = { terms: [], labels: [], trackers: [], paths: [], statuses: [], comparisons: [] };
  for (const raw of input.trim().split(/\s+/)) {
    if (!raw) continue;
    const comparison = raw.match(/^(ratio|size)(>=|<=|>|<)(.+)$/i);
    if (comparison) {
      const value = comparison[1].toLocaleLowerCase() === "size" ? parseSize(comparison[3]) : Number(comparison[3]);
      if (value !== null && Number.isFinite(value)) result.comparisons.push({ field: comparison[1].toLocaleLowerCase() as "ratio" | "size", operator: comparison[2] as Comparison["operator"], value });
      continue;
    }
...
export class TorrentSearchIndex {
  ...
  update(torrent: Torrent) {
    this.entries.set(torrent.hash, {
      name: normalized(torrent.name), hash: normalized(torrent.hash),
      path: normalized(`${torrent.basePath} ${torrent.directory}`), tracker: normalized(torrent.trackerHost),
      message: normalized(torrent.message), label: normalized(torrent.label),
    });
  }

  remove(hash: string) { this.entries.delete(hash); }

  matches(torrent: Torrent, query: ParsedQuery) {
    const entry = this.entries.get(torrent.hash);
    if (!entry) return false;
    const text = (term: string) => entry.name.includes(term) || entry.hash.startsWith(term) || entry.path.includes(term) || entry.tracker.includes(term) || entry.message.includes(term);
    return query.terms.every(text)
      ...
      && query.comparisons.every((rule) => compare(rule.field === "ratio" ? torrent.ratio : torrent.sizeBytes, rule.operator, rule.value));
  }
}
